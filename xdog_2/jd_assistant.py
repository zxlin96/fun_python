...
class Assistant(object):
    ...
    @deprecated
    def exec_seckill_by_time_stamp(self, sku_ids, buy_time, url,retry=4, interval=4, num=1, fast_mode=True):
        """定时抢购
        :param sku_ids: 商品id，多个商品id用逗号进行分割，如"123,456,789"
        :param buy_time: 下单时间，例如：'2018-09-28 22:45:50.000'
        :param retry: 抢购重复执行次数，可选参数，默认4次
        ...
        t = Timer(buy_time=buy_time)
        t.start_by_time_stamp(time_stamp=t.get_time_stamp(),url=url)
    ...
    def exec_reserve_seckill_by_time(self, sku_id, buy_time, retry=4, interval=4, num=1):
        ...
        for count in range(1, retry + 1):
            ...
            logger.info('执行结束，提交订单失败！')
    @check_login
    def exec_reserve_seckill_by_time_stamp(self, sku_id, buy_time, url,retry=4, interval=4, num=1):
        """定时抢购`预约抢购商品`
